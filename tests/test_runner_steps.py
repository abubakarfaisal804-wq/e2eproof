...
def test_browser_step_matrix(tmp_path: Path) -> None:
    with sync_playwright() as playwright, httpx.Client() as client:
        ...
        run_step({"type": "browser.fill", "target": {"label": "Email"}, "value": "{{email}}"}, runtime, c, client)
        assert page.locator("#email").input_value() == "new@example.com"
        run_step({"type": "browser.assert_value", "target": "#email", "equals": "new@example.com"}, runtime, c, client)
        run_step({"type": "browser.assert_value", "target": "#email", "contains": "@example"}, runtime, c, client)
        run_step({"type": "browser.assert_value", "target": "#email", "matches": r"^new@"}, runtime, c, client)
        with pytest.raises(StepExecutionError, match="expected"):
            run_step({"type": "browser.assert_value", "target": "#email", "equals": "bad"}, runtime, c, client)
        ...
        run_step({"type": "browser.check", "target": "#check", "checked": False}, runtime, c, client)
        assert not page.locator("#check").is_checked()

        run_step({"type": "browser.press", "target": "#email", "key": "{{key}}"}, runtime, c, client)
        run_step({"type": "browser.press", "key": "Escape"}, runtime, c, client)
        run_step({"type": "browser.wait", "milliseconds": 1}, runtime, c, client)
        run_step({"type": "browser.wait", "target": "#status", "state": "visible"}, runtime, c, client)

        run_step({"type": "browser.assert_text", "target": "#status", "equals": "Ready now"}, runtime, c, client)
        run_step({"type": "browser.assert_text", "target": "#status", "contains": "Ready"}, runtime, c, client)
        run_step({"type": "browser.assert_text", "target": "#status", "not_contains": "Failed"}, runtime, c, client)
        run_step({"type": "browser.assert_text", "target": "#status", "matches": "Ready\\s+now"}, runtime, c, client)
        with pytest.raises(StepExecutionError, match="unexpectedly"):
            run_step({"type": "browser.assert_text", "target": "#status", "not_contains": "Ready"}, runtime, c, client)
        with pytest.raises(StepExecutionError, match="does not match"):
            run_step({"type": "browser.assert_text", "target": "#status", "matches": "Never"}, runtime, c, client)

        run_step({"type": "browser.assert_visible", "target": "#status", "visible": True}, runtime, c, client)
        run_step({"type": "browser.assert_visible", "target": "#hidden", "visible": False}, runtime, c, client)
        run_step({"type": "browser.assert_count", "target": ".item", "equals": 2}, runtime, c, client)
        run_step({"type": "browser.assert_count", "target": ".item", "minimum": 1, "maximum": 3}, runtime, c, client)
        with pytest.raises(StepExecutionError, match="expected 1"):
            run_step({"type": "browser.assert_count", "target": ".item", "equals": 1}, runtime, c, client)

        page.locator("#button").evaluate("el => el.addEventListener('click', () => document.querySelector('#status').textContent='Saved')")
        run_step({"type": "browser.click", "target": {"role": "button", "name": "Save"}}, runtime, c, client)
        assert page.locator("#status").inner_text() == "Saved"

        _, extract = run_step({"type": "browser.extract", "target": "#status", "variable": "saved_text"}, runtime, c, client)
        assert state.context_values["saved_text"] == "Saved"
        page.locator("#status").evaluate("el => el.setAttribute(\"data-x\", \"42\")")
        run_step({"type": "browser.extract", "target": "#status", "variable": "attr", "attribute": "data-x"}, runtime, c, client)
        assert state.context_values["attr"] == "42"

        _, shot = run_step({"type": "browser.screenshot", "name": "matrix", "full_page": False}, runtime, c, client)
        assert Path(shot["artifact_path"]).exists()
        _, accessibility = run_step({"type": "browser.audit_accessibility", "maximum_violations": 0}, runtime, c, client)
        assert accessibility["count"] == 0
        _, performance = run_step({"type": "browser.assert_performance", "max_load_ms": 999999}, runtime, c, client)
        ...
        _, set_details = run_step({"type": "set.variable", "variable": "nested", "value": "{{email}}"}, runtime, c, client)
        assert set_details["value"] == "new@example.com"

        state.network.extend(
            [
                NetworkRecord(kind="request", url="https://example.com/api", method="POST", request_body="hello"),
                NetworkRecord(kind="response", url="https://example.com/api", method="POST", status=201, response_body='{"ok":true}'),
            ]
        )
        _, net = run_step(
            {"type": "network.assert", "kind": "response", "url_contains": "/api", "method": "POST", "status": 201, "minimum": 1, "maximum": 1},
            runtime, c, client,
        )
        assert net["matched"] == 1
        with pytest.raises(StepExecutionError, match="minimum"):
            run_step({"type": "network.assert", "url_contains": "/nope", "minimum": 1}, runtime, c, client)

        assert _step_screenshot_mode(STEP.validate_python({"type": "browser.wait", "milliseconds": 1, "evidence": "always"}), c) == "always"
        context.close()
        browser.close()
...
def test_observers_and_policy_gates() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        state4 = RuntimeState(context_values={}, network=[NetworkRecord(kind="response", url="https://example.com", method="GET", response_body="mock response")])
